...
#include "main.hpp"
#include "parser.hpp"
#include "compilation-stages/0_register_types.hpp"
#include "compilation-stages/1_register_contents.hpp"
...
class stdout_reporter : public leaf::compilation::reporter
{
protected:
    void do_report(leaf::compilation::message_base& msg) const override
    {
        std::cout << msg.file << ":";

        switch (msg.severity)
        {
            case leaf::compilation::eseverity::fatal: std::cout << "[FTL]"; break;
            case leaf::compilation::eseverity::error: std::cout << "[ERR]"; break;
            case leaf::compilation::eseverity::warning: std::cout << "[WRN]"; break;
            case leaf::compilation::eseverity::info: std::cout << "[INF]"; break;
            case leaf::compilation::eseverity::verbose: std::cout << "[VRB]"; break;
            case leaf::compilation::eseverity::trace: std::cout << "[TRC]"; break;
            case leaf::compilation::eseverity::debug: std::cout << "[DBG]"; break;
        }
        std::cout << "[L" << msg.line << "]"
            << "[C" << msg.column << "]"
            << "[F" << msg.file << "]"
...
int main()
{
    ...
    if (file.has_value())
    {
        leaf::parsing::instance i(*file, "NA");
        ...
        stdout_reporter r;
        leaf::compilation::compilation_unit c(r, res, *file);
        c.register_compilation_stage(std::make_shared<leaf::compilation::stages::register_types>());
        c.register_compilation_stage(std::make_shared<leaf::compilation::stages::register_contents>());
        c.execute();
